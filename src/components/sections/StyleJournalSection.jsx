        ...
        {active && (
          <motion.div initial={{ opacity: 0, scale: 0.85, y: 4 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.85 }}
            ...
            <a href={`https://wa.me/33664826920?text=${waMsg}`} target="_blank" rel="noopener noreferrer"
              ...
              <span style={{ fontFamily: "'Montserrat', sans-serif", fontSize: "7px", letterSpacing: "0.25em", color: "#25D366", textTransform: "uppercase" }}>{t("ask_whatsapp")}</span>
            </a>
          </motion.div>
        )}
      </AnimatePresence>
  ...
  return (
    <section ref={ref} style={{ background: "#0a0602", paddingTop: "4.5rem" }}>
      <motion.div initial={{ opacity: 0, y: 16 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.7 }}
        ...
        <p style={{ fontFamily: "'Cormorant Garamond', serif", fontStyle: "italic", fontSize: "12px", color: "rgba(245,240,232,0.4)", marginTop: "8px" }}>{t("style_journal_hint")}</p>
      </motion.div>
