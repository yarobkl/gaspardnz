    ...
    <section ref={node => { ref.current = node; if (refEl) refEl.current = node; }} style={{ background: "#0d1b3e", padding: "5rem 0 6rem", overflow: "hidden", position: "relative" }}>
      <div ref={ref} style={{ padding: "0 1.4rem", position: "relative" }}>
        <div style={{ position: "relative" }}>
          ...
          <motion.p initial={{ opacity: 0, y: 12 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ delay: 0.5 }} style={{ position: "relative", fontFamily: "'Cormorant Garamond', serif", fontSize: "clamp(0.95rem, 3.8vw, 1.1rem)", fontWeight: 300, color: "rgba(245,240,232,0.75)", lineHeight: 1.7, fontStyle: "italic", marginBottom: "1.4rem" }}>{t("formules_sub")}</motion.p>
          {formules.length > 1 && (
            <motion.p initial={{ opacity: 0, y: 10 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ delay: 0.6 }} style={{ position: "relative", fontFamily: "'Montserrat', sans-serif", fontSize: "11px", lineHeight: 1.8, color: "rgba(184,151,62,0.85)", borderLeft: `2px solid ${GOLD}`, paddingLeft: "0.9rem", marginBottom: "3rem", maxWidth: "440px" }}>{t("formules_diff")}</motion.p>
          )}
        </div>
