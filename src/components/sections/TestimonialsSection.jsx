...
const TestimonialsSection = () => {
  ...
  return (
    <section ref={ref} style={{ background: "#0a0602", padding: "4.5rem 0 5rem" }}>
      <motion.div initial={{ opacity: 0, y: 16 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.7 }} style={{ padding: "0 1.4rem", marginBottom: "2.4rem" }}>
      ...
      <div style={{ padding: "0 1.4rem", display: "flex", flexDirection: "column", gap: "1.2rem" }}>
        {reviews.map((r, i) => (
          <motion.div key={r.id} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-6% 0px" }} transition={{ duration: 0.6, delay: i * 0.07, ease: [0.16, 1, 0.3, 1] }}
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "10px" }}>
              ...
              <span style={{ fontFamily: "'Montserrat', sans-serif", fontSize: "7px", letterSpacing: "0.2em", color: GOLD, textTransform: "uppercase" }}>{r.platform}</span>
            </div>
