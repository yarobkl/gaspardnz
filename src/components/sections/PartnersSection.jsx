...
const COMPACT_COPY = {
  FR: { all: "Voir tous nos partenaires", trust: "Un réseau sélectionné pour accompagner vos événements." },
  EN: { all: "View all partners", trust: "A selected network to support your events." },
  ES: { all: "Ver todos los socios", trust: "Una red seleccionada para acompañar tus eventos." },
...
const PartnersSection = ({ refEl, compact = false, onShowAll }) => {
  ...
  const displayPartners = useMemo(() => {
    if (!compact) return partners;
    const real = partners.filter((partner) => !partner.placeholder && partner.status !== "inactive");
    const sourceList = real.length ? real : partners;
    return [...sourceList]
  ...
  return (
    <>
      <section ref={ref} style={{ background: "#0a0602", padding: compact ? "3.2rem 0 3.6rem" : "5rem 0 6rem", color: CREAM }}>
        <div style={{ maxWidth: "1200px", margin: "0 auto", padding: "0 1.4rem" }}>
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={inView ? { opacity: 1, y: 0 } : { opacity: 0, y: 20 }}
            transition={{ duration: 0.7 }}
            style={{ marginBottom: compact ? "1.8rem" : "3.5rem" }}>
            ...
            <p style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "clamp(16px, 4vw, 20px)", fontStyle: "italic", color: "rgba(245,240,232,0.8)", margin: 0, letterSpacing: "0.02em" }}>
              {compact ? compactCopy.trust : (t("partners_subtitle") || "Les meilleurs professionnels pour votre événement")}
            </p>
          </motion.div>

          <div style={{ display: "grid", gridTemplateColumns: compact ? "repeat(2,minmax(0,1fr))" : "repeat(auto-fit, minmax(280px, 1fr))", gap: compact ? ".75rem" : "2rem", marginBottom: "1rem" }}>
            {displayPartners.map((partner, idx) => (
              <motion.div
                ...
                style={{ background: "rgba(184,151,62,0.03)", border: `1px solid ${GOLD}33`, borderRadius: "8px", padding: compact ? ".9rem" : "1.8rem", position: "relative", overflow: "hidden" }}>
                ...
                {partner.logo && !partner.placeholder ? (
                  <div style={{ height: compact ? "62px" : "120px", display: "flex", alignItems: "center", justifyContent: "center", marginBottom: compact ? ".8rem" : "1.5rem", borderBottom: `1px solid ${GOLD}22` }}>
          ...
          {compact ? (
            <button
              type="button"
              onClick={onShowAll}
              style={{ width: "100%", minHeight: 48, marginTop: ".9rem", border: `1px solid ${GOLD}66`, background: "rgba(184,151,62,.07)", color: GOLD, fontFamily: "'Montserrat',sans-serif", fontSize: 9, fontWeight: 600, letterSpacing: ".18em", textTransform: "uppercase", cursor: "pointer", touchAction: "manipulation" }}>
              {compactCopy.all}
            </button>
          ) : (
