...
export default function ShowroomMobile({ refEl, onCatalogue, compact = false }) {
  ...
  const compactMode = compact || isCompactMobile;
  ...
  return <section ref={refEl} style={{background:"#f5f0e8",overflow:"hidden"}}>
    ...
    <div onClick={()=>setAlbumOpen(true)} style={{position:"relative",height:compactMode ? "58vw" : "85vw",minHeight:compactMode ? 220 : 340,maxHeight:compactMode ? 320 : 560,overflow:"hidden",cursor:"pointer",background:"#0a0602"}}>
      ...
      <div style={{position:"absolute",left:"1.2rem",right:"1.2rem",bottom:compactMode ? "1rem" : "1.5rem"}}><h3 style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:compactMode ? "clamp(2rem,10vw,3.4rem)" : "clamp(2.5rem,12vw,4.5rem)",letterSpacing:".06em",lineHeight:.9,color:"#faf7f2",margin:0,textShadow:"0 3px 24px rgba(0,0,0,.55)"}}>{title}</h3><p style={{fontFamily:"'Cormorant Garamond',serif",fontStyle:"italic",color:GOLD,fontSize:"1rem",margin:".45rem 0 0"}}>{photos[cur]?.label}</p></div>
    </div>

    <div ref={ref} style={{padding:compactMode ? "1.6rem 1.4rem 2rem" : "3rem 1.4rem 4rem",maxWidth:900,margin:"0 auto"}}>
      ...
      <motion.h2 initial={{opacity:0,y:18}} animate={inView?{opacity:1,y:0}:{}} transition={{delay:.1}} style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:compactMode ? "clamp(34px,10vw,48px)" : "clamp(44px,14vw,76px)",lineHeight:.9,letterSpacing:".04em",color:TEXT,margin:compactMode ? "0 0 .9rem" : "0 0 1.4rem"}}>{t("custom_art_title")}</motion.h2>
      {!compactMode && <motion.p initial={{opacity:0,y:16}} animate={inView?{opacity:1,y:0}:{}} transition={{delay:.2}} style={{fontFamily:"'Cormorant Garamond',serif",fontSize:"clamp(1rem,4vw,1.2rem)",fontStyle:"italic",lineHeight:1.8,color:"rgba(28,18,8,.75)",margin:"0 0 2rem"}}>{description}</motion.p>}
