  ...
  return <Portal><motion.div ref={trap} role="dialog" aria-modal="true" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} onClick={onClose} style={{position:"fixed",inset:0,zIndex:9999,background:"rgba(8,5,2,.96)",display:"grid",placeItems:"center",padding:"1rem"}}>
    ...
    <motion.div onClick={(e)=>e.stopPropagation()} initial={{y:20,opacity:0}} animate={{y:0,opacity:1}} exit={{y:20,opacity:0}} style={{width:"min(92vw,560px)"}}>
      ...
      <div style={{position:"relative",borderRadius:16,overflow:"hidden",border:"1px solid rgba(184,151,62,.26)",background:"#0a0602"}}>
        <AnimatePresence mode="wait"><motion.img key={current?.src} src={current?.src} alt={current?.label || title} width="900" height="1200" initial={{opacity:0,scale:1.02}} animate={{opacity:1,scale:1}} exit={{opacity:0,scale:.99}} transition={{duration:.3}} style={{width:"100%",height:"min(66vh,680px)",objectFit:"contain",display:"block"}} /></AnimatePresence>
...
export default function ShowroomMobile({ refEl, onCatalogue }) {
  ...
  const showAlbumShowcase = !isCompactMobile;

  useEffect(()=>{ if (cur >= photos.length) setCur(0); },[cur,photos.length]);
  useEffect(()=>{
    if (isCompactMobile || photos.length < 2) return undefined;
    const id=window.setInterval(()=>setCur((v)=>(v+1)%photos.length),4500);
  ...
  return <section ref={refEl} style={{background:CREAM,overflow:"hidden"}}>
    ...
    <div ref={ref} style={{padding:isCompactMobile?"3.2rem 1.4rem 3.8rem":"3rem 1.4rem 4rem",maxWidth:900,margin:"0 auto"}}>
