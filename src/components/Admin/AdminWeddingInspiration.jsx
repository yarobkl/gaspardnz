...
export default function AdminWeddingInspiration() {
  ...
  useEffect(() => { load(); }, []);
  const edit = (row) => { setForm(row); setAlbumText(albumToText(row.album)); scrollToAdminEditor(); };
  const toggle = async (row) => { try { await setPublished("wedding_inspirations", row.id, !row.published); await load(); } catch (e) { setError(e?.message || "Impossible de changer la visibilité."); } };
