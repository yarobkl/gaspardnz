  ...
  return <div>
    ...
    <div className="gnz-split">
      ...
      <aside className="gnz-card gnz-editor" id="gnz-admin-editor"><header className="gnz-card-header"><div className="gnz-card-title"><strong>{form.id?"Modifier l'album":"Créer un album"}</strong><span>Chaque modification publiée est reprise automatiquement par le site.</span></div></header><form className="gnz-card-body gnz-editor-grid" onSubmit={save}>
        ...
        <div style={{borderTop:"1px solid rgba(205,169,75,.16)",paddingTop:12}}><div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:8,marginBottom:10}}><strong>Photos</strong><button type="button" className="gnz-secondary-button" onClick={()=>setForm({...form,items:[...form.items,{src:"",label:"",width:1200,height:1500}]})}>Ajouter une photo</button></div>{form.items.map((item,index)=><div key={index} style={{border:"1px solid rgba(205,169,75,.14)",padding:10,borderRadius:8,marginBottom:8}}><MediaUploadField label="Photo" value={item.src} onChange={(url) => updateItem(index,"src",url)} uploadSection={form.section_key} /><label className="gnz-field">Légende<input className="gnz-input" value={item.label||""} onChange={(e)=>updateItem(index,"label",e.target.value)}/></label><div style={{display:"flex",gap:6,flexWrap:"wrap"}}><button type="button" className="gnz-secondary-button" onClick={()=>move(index,-1)} disabled={index===0}>Monter</button><button type="button" className="gnz-secondary-button" onClick={()=>move(index,1)} disabled={index===form.items.length-1}>Descendre</button><button type="button" className="gnz-secondary-button" onClick={()=>removeItem(index)}>Retirer</button></div></div>)}</div>
