...
export default function AdminVIPClients() {
  ...
  useEffect(() => { load(); }, []);
  const edit = (row) => { setForm(row); setAlbumText(lines(row.album)); scrollToAdminEditor(); };
  const toggle = async (row) => { try { await setPublished("vip_clients", row.id, !row.published); await load(); } catch (e) { setError(e?.message || "Impossible de changer la visibilité."); } };
