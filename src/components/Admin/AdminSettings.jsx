...
export default function AdminSettings() {
  ...
  return <div>
    ...
    <article className="gnz-card" style={{marginTop:12}}><header className="gnz-card-header"><div className="gnz-card-title"><strong>Intégrations</strong><span>Une intégration non connectée n'alimente aucun faux KPI.</span></div></header><div className="gnz-card-body"><div className="gnz-integration-grid">{integrations.map((item) => {
      const isGoogle = item.provider === "google_analytics" || item.provider === "google_search_console";
      const isConnected = item.status === "connected";
      return <div className="gnz-integration-card" key={item.provider}><div className="gnz-integration-card-head"><div><h3>{LABELS[item.provider] || item.provider}</h3><p>{item.account_label || "Compte non configuré"}<br/>{item.site_url || ""}</p></div>{isGoogle && !isConnected ? <a className="gnz-secondary-button" href="/admin/seo">Connecter Google</a> : <span className={`gnz-status ${isConnected ? "success" : item.status === "partial" ? "info" : "warning"}`}>{isConnected ? "Connecté" : item.status === "partial" ? "Partiel" : "À connecter"}</span>}</div>{item.last_sync_at && <p>Dernière synchro : {new Date(item.last_sync_at).toLocaleString("fr-FR")}</p>}{item.last_error && <p style={{color:"var(--gnz-danger)"}}>Erreur : {item.last_error}</p>}</div>;
    })}</div></div></article>
