...
function GeneralEditor({ initial, fields, labels, onSave, disabled }) {
  const [value, setValue] = useState(initial || {});
  useEffect(() => setValue(initial || {}), [initial]);
  return <div className="gnz-card-body gnz-editor-grid">{fields.map((field) => <label className="gnz-field" key={field}>{labels[field] || field}<input className="gnz-input" value={value[field] || ""} onChange={(e) => setValue({ ...value, [field]: e.target.value })} /></label>)}<button className="gnz-primary-button" disabled={disabled} onClick={() => onSave(value)}>Enregistrer</button></div>;
}
