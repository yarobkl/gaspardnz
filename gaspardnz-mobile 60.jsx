            ...
            return (
              <motion.div key={i}
                ...
                <div style={{ borderRadius: "16px", background: c.gradient, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", border: isActive ? `1px solid rgba(184,151,62,0.5)` : "1px solid rgba(184,151,62,0.15)", position: "relative", overflow: "hidden", aspectRatio: "3/4", boxShadow: isActive ? "0 20px 60px rgba(0,0,0,0.7)" : "none", transition: "border 0.4s, box-shadow 0.4s" }}>
                  ...
                  <p style={{ fontFamily: "'Montserrat', sans-serif", fontSize: "9px", letterSpacing: "0.15em", color: isActive ? "rgba(250,247,242,0.85)" : "rgba(250,247,242,0.4)", textTransform: "uppercase", transition: "color 0.4s", position: "absolute", bottom: "12px", left: 0, right: 0, textAlign: "center", zIndex: 1 }}>{c.event}</p>
                </div>
...
const CommunauteSection = () => {
  ...
  const inView = useInView(ref, { once: true, margin: "-8% 0px" });
  return (
    <section ref={ref} style={{ background: "#0a0602", padding: "5rem 0 5.5rem", position: "relative", overflow: "hidden" }}>
      {/* Cercles déco */}
      <div style={{ position: "absolute", top: "-80px", right: "-80px", width: "260px", height: "260px", borderRadius: "50%", border: "1px solid rgba(37,211,102,0.08)", pointerEvents: "none" }} />
      <div style={{ position: "absolute", bottom: "-60px", left: "-60px", width: "200px", height: "200px", borderRadius: "50%", border: "1px solid rgba(184,151,62,0.1)", pointerEvents: "none" }} />

      <div style={{ padding: "0 1.6rem", textAlign: "center" }}>

        {/* Surtitle */}
        <motion.p initial={{ opacity: 0, y: 12 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6 }}
          style={{ fontFamily: "'Montserrat', sans-serif", fontSize: "7px", letterSpacing: "0.55em", color: "#25D366", textTransform: "uppercase", marginBottom: "16px" }}>
          CANAL WHATSAPP · GASPARDNZ
        </motion.p>

        {/* Titre */}
        <motion.p initial={{ opacity: 0, y: 16 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.7, delay: 0.08 }}
          style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "clamp(2rem, 9vw, 3rem)", fontWeight: 300, color: "#faf7f2", lineHeight: 1.15, marginBottom: "1rem" }}>
        ...
        <motion.div initial={{ scaleX: 0 }} animate={inView ? { scaleX: 1 } : {}} transition={{ duration: 0.6, delay: 0.2 }}
          style={{ width: "48px", height: "1px", background: `linear-gradient(90deg, transparent, ${GOLD}, transparent)`, margin: "0 auto 1.6rem" }} />

        {/* Description */}
        <motion.p initial={{ opacity: 0 }} animate={inView ? { opacity: 1 } : {}} transition={{ duration: 0.6, delay: 0.25 }}
          style={{ fontFamily: "'Cormorant Garamond', serif", fontStyle: "italic", fontSize: "13px", color: "rgba(245,240,232,0.55)", lineHeight: 1.8, marginBottom: "2.4rem", maxWidth: "300px", margin: "0 auto 2.4rem" }}>
          Nouveaux looks, arrivages exclusifs, coulisses — reçois tout en avant-première directement sur WhatsApp.
        </motion.p>

        {/* Badge avantages exclusifs */}
        <motion.div initial={{ opacity: 0, scale: 0.85 }} animate={inView ? { opacity: 1, scale: 1 } : {}} transition={{ duration: 0.5, delay: 0.3, type: "spring", stiffness: 260 }}
          style={{ display: "inline-flex", alignItems: "center", gap: "12px", background: "rgba(184,151,62,0.1)", border: `1px solid rgba(184,151,62,0.3)`, borderRadius: "40px", padding: "12px 20px", marginBottom: "2.4rem" }}>
          <span style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "22px", fontWeight: 600, color: GOLD, lineHeight: 1.2 }}>✦</span>
          <div style={{ textAlign: "left" }}>
            <p style={{ fontFamily: "'Montserrat', sans-serif", fontSize: "8px", fontWeight: 600, color: "#faf7f2", letterSpacing: "0.08em", marginBottom: "2px" }}>AVANTAGES EXCLUSIFS</p>
            <p style={{ fontFamily: "'Montserrat', sans-serif", fontSize: "7px", color: "rgba(245,240,232,0.45)", letterSpacing: "0.05em" }}>réservés aux membres de la communauté</p>
          </div>
        </motion.div>

        {/* Bouton WhatsApp */}
        <motion.div initial={{ opacity: 0, y: 12 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6, delay: 0.38 }}>
          <motion.a href={WA_CHANNEL_URL} target="_blank" rel="noopener noreferrer"
            whileTap={{ scale: 0.96 }}
            style={{ display: "inline-flex", alignItems: "center", gap: "10px", background: "#25D366", padding: "14px 28px", borderRadius: "32px", textDecoration: "none", boxShadow: "0 8px 32px rgba(37,211,102,0.25)" }}>
            <svg width="20" height="20" viewBox="0 0 24 24" fill="white"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347z"/><path d="M12 0C5.373 0 0 5.373 0 12c0 2.127.558 4.122 1.532 5.852L.057 23.032a.75.75 0 0 0 .921.921l5.18-1.475A11.943 11.943 0 0 0 12 24c6.627 0 12-5.373 12-12S18.627 0 12 0zm0 21.75a9.713 9.713 0 0 1-4.953-1.355l-.355-.212-3.676 1.047 1.047-3.608-.23-.372A9.718 9.718 0 0 1 2.25 12C2.25 6.615 6.615 2.25 12 2.25S21.75 6.615 21.75 12 17.385 21.75 12 21.75z"/></svg>
            <span style={{ fontFamily: "'Montserrat', sans-serif", fontSize: "9px", letterSpacing: "0.3em", color: "white", textTransform: "uppercase", fontWeight: 600 }}>Rejoindre le canal</span>
          </motion.a>
        </motion.div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "1.2rem" }}>
          ...
          {[
            ...
            <div key={i} style={{ borderRadius: "12px", overflow: "hidden", border: "1px solid rgba(184,151,62,0.15)" }}>
              <img src={p.src} alt={p.label} style={{ width: "100%", height: "auto", display: "block" }} />
