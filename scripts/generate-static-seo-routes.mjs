...
const nav = `<nav aria-label="Navigation GaspardNZ" style="display:flex;gap:18px;flex-wrap:wrap;margin-top:30px"><a href="/styliste-mariage-homme-paris">Styliste mariage homme Paris</a><a href="/services">Services</a><a href="/lookbook">Lookbook</a><a href="/galerie">Galerie</a><a href="/a-propos">À propos</a><a href="/contact">Contact</a></nav>`;
...
const faqSchema = `<script type="application/ld+json">${JSON.stringify({ "@context": "https://schema.org", "@type": "FAQPage", mainEntity: [{ "@type": "Question", name: "Quand prendre rendez-vous avec un styliste avant le mariage ?", acceptedAnswer: { "@type": "Answer", text: "Le plus tôt possible lorsque plusieurs tenues ou temps forts sont prévus. Le premier échange sert à poser le calendrier et les décisions à prendre." } }, { "@type": "Question", name: "Peut-on prévoir plusieurs looks pour le mariage ?", acceptedAnswer: { "@type": "Answer", text: "Oui. Mairie, cérémonie, dîner et soirée peuvent demander des niveaux de formalité différents tout en gardant une cohérence entre les silhouettes." } }, { "@type": "Question", name: "L’accompagnement est-il uniquement pour le marié ?", acceptedAnswer: { "@type": "Answer", text: "Non. Il peut aussi concerner un témoin, un père ou un proche qui occupe un rôle important dans le mariage." } }] })}</script>`;
...
for (const [path, page] of Object.entries(pages)) {
  ...
  if (path === "/styliste-mariage-homme-paris") html = html.replace("</head>", `${faqSchema}\n</head>`);
